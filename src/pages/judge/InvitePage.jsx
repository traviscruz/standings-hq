...
export default function InvitePage() {
  ...
  const [hoveredButton, setHoveredButton] = useState(null); // id-action
    ...
    <div className="slide-up-anim">
      <div style={pageHeaderStyle}>
        <div>
          ...
          <h1 style={pageTitleStyle}>Pending Requests</h1>
          <p style={pageDescriptionStyle}>Manage your judging invitations. Accepted events will appear in your live dashboard.</p>
        </div>
      </div>

      <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : 'repeat(auto-fill, minmax(400px, 1fr))', gap: '24px', alignItems: 'start' }}>
        {invitations.length === 0 ? (
          <div style={{ ...widgetCardStyle('empty'), textAlign: 'center', padding: '60px', gridColumn: '1 / -1', flexDirection: 'column', alignItems: 'center' }}>
            ...
            <p style={{ fontSize: '16px', color: colors.inkSoft, fontWeight: 600, margin: 0 }}>No pending invitations found.</p>
          ...
          invitations.map(inv => (
            <div 
              key={inv.id} 
              style={widgetCardStyle(inv.id)}
              onMouseEnter={() => setHoveredCard(inv.id)}
              onMouseLeave={() => setHoveredCard(null)}
            >
              <div style={{
                width: '48px', height: '48px', borderRadius: '12px',
                background: '#F8FAFC', color: colors.navy,
